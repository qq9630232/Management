...
public class CarAddActivity_bak extends BaseActivity<CarAddPresenter> implements CarAddPresenter.View, View.OnClickListener {
    ...
    private static final int REQUEST_CODE_CARD_JIDONG_LICENSE1 = 123;
    ...
    private TextView mTvCardJiDong1;
    private ImageView mIvCardJiDong1;
    private ImageView mCloseJiDong1;
    ...
    protected void onInitView() {
        ...
        mTvCardJiDong1 = findViewById(R.id.tv_card_ji_dong1);
        mIvCardJiDong1 = findViewById(R.id.iv_card_ji_dong1);
        mCloseJiDong1 = findViewById(R.id.close_ji_dong1);
        ...
        mReJiDongPic1 = findViewById(R.id.re_ji_dong_pic1);
        mReJiDongPic1.setOnClickListener(this);
...
//                certificateRegistration.setDisplacement(mEtDengJiPaiLiang.getText().toString());
     ...
     * 机动车登记证1
     */
    private void taskJidong1Photo() {
        jidongPath1 = "jidong1_" + System.currentTimeMillis();
        Intent intent = new Intent(this, CameraActivity.class);
        intent.putExtra(CameraActivity.KEY_OUTPUT_FILE_PATH,
                FileUtil.getSaveFile(getApplication(), jidongPath1).getAbsolutePath());
        intent.putExtra(CameraActivity.KEY_CONTENT_TYPE,
                CameraActivity.CONTENT_TYPE_GENERAL);
        startActivityForResult(intent, REQUEST_CODE_CARD_JIDONG_LICENSE1);
    }
    /**
     * 机动车登记证2
     */
...
//                            mTvCard2.setVisibility(View.GONE);
        ...
        // 识别成功回调，机动车登记证识别1
        if (requestCode == REQUEST_CODE_CARD_JIDONG_LICENSE1 && resultCode == Activity.RESULT_OK) {
            final String filePath = FileUtil.getSaveFile(getApplicationContext(), jidongPath1).getAbsolutePath();

            RecognizeService.recCustom(this, filePath, TEMPLATE_JIDONG_SIGN1, 0,
                    new RecognizeService.ServiceListener() {
                        @Override
                        public void onResult(String result) {
                            ToastUtils.popUpToast("此识别结果仅供参考，请仔细比对检查");
                            boolean json = StringUtils.isJson(result);
                            if (!json) {
                                //ToastUtils.popUpToast(result.toString());
                                ToastUtils.popUpToast("识别错误，请重新上传");
                                return;
                            }
                            ...
                            mIvCardJiDong1.setVisibility(View.VISIBLE);
                            //mTvCardJiDong.setVisibility(View.GONE);
                            Glide.with(getContext()).load(filePath).into(mIvCardJiDong1);
                            mPresenter.upload(new File(filePath), UPLOAD_JIDONG);
                        }
                    });
        }
        // 识别成功回调，机动车登记证识别2
        if (requestCode == REQUEST_CODE_CARD_JIDONG_LICENSE2 && resultCode == Activity.RESULT_OK) {
            final String filePath = FileUtil.getSaveFile(getApplicationContext(), jidongPath2).getAbsolutePath();

            RecognizeService.recCustom(this, filePath, TEMPLATE_JIDONG_SIGN2, 0,
                    new RecognizeService.ServiceListener() {
                        @Override
                        public void onResult(String result) {
                            ToastUtils.popUpToast("此识别结果仅供参考，请仔细比对检查");
                            ...
                            mIvCardJiDong2.setVisibility(View.VISIBLE);
                            //mTvCardJiDong.setVisibility(View.GONE);
                            Glide.with(getContext()).load(filePath).into(mIvCardJiDong2);
                            mPresenter.upload(new File(filePath), UPLOAD_JIDONG);
                        }
                    });
        }
