...
public class DemoHelper {
    ...
    public static int getAvatarResource(String username, int defaultDrawable) {
        User user = UserRepository.getInstance().getUserByUsername(username);
        if(user == null) {
            return defaultDrawable == 0 ? R.drawable.ic_launcher_background : defaultDrawable;
        }
        ...
        return resId == 0 ? R.drawable.ic_launcher_background : resId;
    }
...
//        DemoDbHelper.getInstance(BaseApplication.getApp()).initDb(EMClient.getInstance().getCurrentUser());
    }
