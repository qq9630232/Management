...
import com.example.freightmanagement.View.User;
import com.example.freightmanagement.View.UserRepository;
...
public class EmClientRepository extends BaseEMRepository {
     ...
     * @return
     */
    public LiveData<Resource<List<String>>> getMembers(String roomId,EMValueCallBack back) {
        return new NetworkOnlyResource<List<String>, List<String>>() {
            @Override
            protected void createCall(@NonNull ResultCallBack<LiveData<List<String>>> callBack) {
                ThreadManager.getInstance().runOnIOThread(()-> {
...
//                        EventBus.getDefault().
                        back.onSuccess(allMembers);
                    } catch (HyphenateException e) {
                        back.onError(e.getErrorCode(), e.getMessage());
                    }
                });
            }
        }.asLiveData();
